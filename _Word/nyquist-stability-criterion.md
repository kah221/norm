...
## 0. この手法を使うことの利点
- [むだ時間](_Word/dead-time.md)を含む系にも適用可能（根軌跡法はむだ時間を含む系に適用不可能）
- 安定の度合いが分かる（根軌跡法では安定の度合いは分からない）
- 閉ループ極を数値的に求める必要がない（根軌跡法では正確にプロットが必要）
- [最小位相系](_Word/minimum-phase-system.md)では，ベクトル軌跡のプロットだけで判別可能
...
- [包囲](_Word/encirclement.md)：閉曲線が任意の点を閉じ込めたときの呼び方．閉曲線に向きがあるので符号付きで表記される
- [囲い込み](_Word/enclosure.md)：最小位相系の場合に用いる概念
...
>   開ループ伝達関数 $\boldsymbol{G} (s)$：ループ部分を含めない「まっすぐ部分」だけの伝達関数  

FBループ部分の伝達関数 $\boldsymbol{H}(s)$ が $1$ ならば，両者は一致する．  
文献間で混同されている場合がある．

### 1.2 なぜ3つも平面があるか??
> ナイキスト安定判別法の厳密な定義では 写像→平行移動 の2段階のプロットを経て，ナイキスト線図を得るため  
> ★実用上，中間の$\Delta(s)$平面は不要  
> ★さらに，システムが「[最小位相系](_Word/minimum-phase-system.md)」であれば，ナイキスト線図も一部だけで足りる
...
- 結局，流れとしては，まず$P$ が分かる．この$P$ に対してNがどんな値であれば～～　という考え方をする
	- 例）$P = 1$ で開ループが不安定なシステムを安定化させたいなら...
		...
		- すなわち，$L(s)$線図が臨界点 $(-1, j0)$ を反時計回り(CCW)に1回「包囲」すればよい．
...
## 2. 判定手順（厳密）
（[最小位相系](_Word/minimum-phase-system.md)の場合は簡略化できる→[こちら](####手順)）  
...
7. $\omega = 0$ と $\omega = \infty$ における振幅と偏角を調べる
	- 安定限界に直接無関係だが，囲い込みの向きで重要になる
	- （画像では飛ばしてしまっている）
8. $\omega_0$ を $(*)$ に代入する（$\boldsymbol{L}(j\omega_0)$ を求める） 
